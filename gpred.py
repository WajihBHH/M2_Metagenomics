...
def read_fasta(fasta_file: Path) -> str:
    ...
    try:
        ...
        sys.exit(f"Error: Could not open file {fasta_file}")
...
def find_stop(stop_regex: Pattern, sequence: str, start: int) -> Union[int, None]:
    ...
    while match:
        # Check if it's in the same reading frame (i.e., distance from start is multiple of 3)
        if (match.start() - start) % 3 == 0:
            return match.start()
        match = stop_regex.search(sequence, match.start() + 1)
    return None
...
def has_shine_dalgarno(shine_regex: Pattern, sequence: str, start: int, max_shine_dalgarno_distance: int) -> bool:
    ...
    """
    region_start = max(0, start - max_shine_dalgarno_distance)
    region = shine_regex.search(sequence, region_start, start -6)
    if region:
        return True
    return False
...
def predict_genes(sequence: str, start_regex: Pattern, stop_regex: Pattern, shine_regex: Pattern, 
    ...
    while seq_len - pos >= min_gap:
            ...
            if pos:
                stop_match = find_stop(stop_regex, sequence, pos)
                
                if stop_match:
                    gene_len = stop_match - pos

                    if gene_len >= min_gene_len:

                        if has_shine_dalgarno(shine_regex, sequence, pos, max_shine_dalgarno_distance):
                            probable_genes.append([pos+1, stop_match +3])
                            pos = stop_match +3 +min_gap
                        else:
                            pos +=1
                    else:
                        pos +=1
...
def main() -> None: # pragma: no cover
    ...
    probable_genes2= []
    for gene in probable_genes_rc:
        rc_start = len(sequence_rc) - gene[1] + 1
        rc_end = len(sequence_rc) - gene[0] + 1
        probable_genes2.append([rc_start, rc_end])

    probable_genes2.sort()

    # Call to output functions
    write_genes_pos(args.predicted_genes_file, probable_genes+probable_genes2)
    write_genes(args.fasta_file, sequence, probable_genes, sequence_rc, probable_genes_rc)
